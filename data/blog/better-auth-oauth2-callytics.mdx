...
- **OpenID Connect Discovery**: Use `discoveryUrl` to automatically fetch endpoints from providers that support `.well-known/openid-configuration`
- **Manual Configuration**: Specify `authorizationUrl`, `tokenUrl`, and `userInfoUrl` directly for providers like Cal.com that don't support discovery
...
The OAuth callback endpoint (`/api/auth/callback/:providerId`) is handled automatically by Better Auth—no custom route handlers needed. The plugin:
...
Better Auth includes a sophisticated auto-refresh mechanism. When the `expires_in` field is present in the token response (RFC 6749 compliant), the library automatically calculates the expiration time:
...
While integrating Cal.com OAuth in Callytics, I discovered that users had to manually re-authenticate every 30 minutes. This was frustrating for the user experience.
...
Without `expires_in`, Better Auth (and other OAuth clients) couldn't determine when to refresh the token, leading to frequent manual re-authentication.
...
- No more manual re-authentication every 30 minutes!
- Full RFC 6749 compliance

## 💡 Key Benefits of Better Auth's Generic OAuth Plugin
