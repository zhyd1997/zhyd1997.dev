...
在我之前的工作中，需要集成 Meta（包括 Facebook 和 Instagram）和 Outlook 的 OAuth 2.0，需要手写 callback、refresh token 逻辑，令人头疼。

现在同样是在 Next.js 中，我在 Callytics 这个项目中集成 Cal.com OAuth 2.0 时，只需用 Better Auth 轻松搞定。
...
Better Auth includes a sophisticated auto-refresh mechanism. Looking at the source code at [`@better-auth/better-auth/files/packages/core/src/oauth2/refresh-access-token.ts` (lines 108-123)](https://github.com/better-auth/better-auth), the library intelligently:
...
- [Better Auth Auto Refresh Source](https://github.com/better-auth/better-auth/blob/main/packages/core/src/oauth2/refresh-access-token.ts)
